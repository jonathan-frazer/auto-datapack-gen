				...
				if isinstance(ability, list):
						...
						for j,subAbility in enumerate(ability):
							...
							if sneakVariant:
								...
								f"{f"scoreboard players set @s {nameShortener(subAbility.get('name',f"SubAbility{j}"),max_length=12)}{i}CD {int(subAbility['cooldown']*2)}" if 'cooldown' in subAbility else ""}"]
								check_lines = [
											"#Perform Arbitrary Cooldown Check here",
											f"{f"execute if score @s {nameShortener(subAbility.get('name',f"SubAbility{j}"),max_length=12)}{i}CD matches 0 run " if 'cooldown' in subAbility else ""}function {datapackParams['namespace']}:{charNamespace}/slot_{i}/{j+1}_{nameShortener(subAbility.get('name',"SubAbility"),max_length=16,type="namespace")}/shiftclick/0_init"
										]
							...
							check_lines = [
											"#Perform Arbitrary Cooldown Check here",
											f"{f"execute if score @s {nameShortener(subAbility.get('name',f"SubAbility{j}"),max_length=12)}{i}CD matches 0 run " if 'cooldown' in subAbility else ""}function {datapackParams['namespace']}:{charNamespace}/slot_{i}/{j+1}_{nameShortener(subAbility.get('name',"SubAbility"),max_length=16,type="namespace")}/rclick/0_init"
										]
				...
				if isinstance(ability,dict):
						...
						for slot in action_slots:
								lines = [
									...
									f"{f"scoreboard players set @s {nameShortener(ability.get('name',f"Ability{i}"),max_length=12)}{i}CD {int(ability['cooldown']*2)}" if 'cooldown' in ability else ""}"
								...
								if 'click' in slot:
									check_lines = [
											"#Perform Arbitrary Cooldown Check here",
											f"{f"execute if score @s {nameShortener(ability.get('name',f"Ability{i}"),max_length=12)}{i}CD matches 0 run " if 'cooldown' in ability else ""}function {datapackParams['namespace']}:{charNamespace}/slot_{i}/{map[slot]}/0_init"
										]
								...
								if 'f-press' in slot:
										check_lines = [
											...
											"",
											"#Check Slot Number(and Arbitrary Cooldown)",
											f"execute if score @s SelectedSlot matches {i} {f"if score @s {nameShortener(ability.get('name',f"Ability{i}"),max_length=12)}{i}CD matches 0" if 'cooldown' in ability else ""}run function {datapackParams['namespace']}:{charNamespace}/slot_{i}/{map[slot]}/0_init"
										]
										with open(os.path.join(os.path.dirname(ability_file_path),"0_check.mcfunction"), 'w') as f:
											f.write("\n".join(check_lines))
